...
public final class SvDiscoverFromLocalAssemblyContigAlignmentsSpark extends GATKSparkTool {
    ...
    public static final class AssemblyContigsClassifiedByAlignmentSignatures {
        ...
        private void writeSAMfilesForUnknown(final String outputPrefix, final JavaRDD<GATKRead> assemblyRawAlignments,
            ...
            final Set<String> ambiguousContigNames = Sets.newHashSet( reasonToContigNames.get(AMBIGUOUS) );
            final Set<String> incompleteContigNames = Sets.newHashSet( reasonToContigNames.get(INCOMPLETE) );
            final Set<String> misAssemblySuspectContigNames = Sets.newHashSet( reasonToContigNames.get(MIS_ASSEMBLY_OR_MAPPING_SUSPECT) );

            final List<GATKRead> contigRawAlignments = assemblyRawAlignments.collect();
            SvDiscoveryUtils.writeSAMRecords(contigRawAlignments, ambiguousContigNames, outputPrefix + "ambiguous.bam", header);
            SvDiscoveryUtils.writeSAMRecords(contigRawAlignments, incompleteContigNames, outputPrefix + "incomplete.bam", header);
            SvDiscoveryUtils.writeSAMRecords(contigRawAlignments, misAssemblySuspectContigNames, outputPrefix + "mis_assembly_or_mapping.bam", header);
        }
    }
